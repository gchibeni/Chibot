...
A lightweight, self-hosted Discord bot built to run beautifully on a Raspberry Pi — multi-server, RAM-optimized, and packed with music, custom triggers, and admin tooling.
...
| `/say`      | Guild    | Build and send a message _(Manage Server)_                             |
| `/attach`   | Guild    | Attach files or images to a message or builder _(Manage Server)_       |
| `/purge`    | Guild    | Purge messages, preserving pinned and starred ones _(Manage Messages)_ |
| `/addicon`  | Guild    | Schedule a themed server icon _(Administrator)_                        |
...
1. Install **[uv](https://docs.astral.sh/uv/)**:
    ...
    > Nothing else to install — uv fetches Python and every dependency for you.
...
That's the whole setup — Python, ffmpeg and every dependency are installed for you. There is no `apt-get` / `brew` / `choco` step, and you never need to activate the virtual environment.

> [!NOTE]
> **On Linux and macOS only**, voice needs libopus. It's usually already present; if voice commands fail, install it:
>
> ```bash
